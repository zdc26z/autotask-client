 ...
 */
class ResourceTimeOffApproverQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ResourceTimeOffApprovers/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ResourceTimeOffApprovers/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ResourceTimeOffApproverCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ResourceTimeOffApprovers/query", $this->toArray());
        }else{
            $response = $this->client->get("ResourceTimeOffApprovers/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ResourceTimeOffApproverCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ResourceTimeOffApproverPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ResourceTimeOffApprovers/query", $this->toArray());
            return new ResourceTimeOffApproverPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ResourceTimeOffApprovers/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ResourceTimeOffApproverPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ResourceTimeOffApprovers/query')
        );
